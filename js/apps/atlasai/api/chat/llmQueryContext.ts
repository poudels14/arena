      ...
      return function addFunctions({ request }) {
        ...
        if (files.length > 0) {
          request.messages.push({
            role: "user",
            content:
              `Here are some files that might be related to the my query. Use this information to if it is applicable, else, ignore the provided content and answer from your own knowledge.` +
              `\n` +
              files
                .map((file) => {
                  return (
