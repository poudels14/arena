...
const AIChat = (props: { assistantId: string; threadId: string }) => {
  ...
  const messages = createMemo(() => {
    if (props.threadId) {
      const messages = state.threads[props.threadId].messages();
      if (!messages) {
        return null;
      }
      return messages;
  ...
  const error = createMemo(() => {
    ...
    return errors.find(
      (e) => e.channelId == props.assistantId && e.threadId == props.threadId
    );
  });
        ...
        <div class="flex-1 max-w-[650px]">
          ...
          >
            <Switch>
                    ...
                    return (
                      <Switch>
                        <Match when={message.metadata.workflow!()}>
                          <PluginWorkflow
                            id={message.metadata.workflow!.id()!}
                          />
                        </Match>
              ...
              <Match when={!assistantState.activeThreadId()}>
                <EmptyAIThread />
              </Match>
            </Switch>
          ...
          <div class="flex p-2 flex-row text-accent-11">
            <Show when={Boolean(props.threadId)}>
              <div class="new-chat flex pr-2 text-xs font-normal text-brand-12/80 border border-brand-12/50 rounded align-middle cursor-pointer select-none bg-white shadow-2xl">
                ...
                <div
                  class="leading-5"
                  onClick={() => navigate(`/assistants/${props.assistantId}`)}
                >
                  New thread
                </div>
              </div>
...
const Chatbox = (props: { thread: Store<Chat.Thread> }) => {
   ...
   */
  createEffect(() => {
    void assistantState.activeThreadId();
    textareaRef?.focus();
  });
