// Copied from https://github.com/jbg/tokio-postgres-rustls
  ...
  S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
  ...
  type Error = io::Error;
  type Future =
    Pin<Box<dyn Future<Output = io::Result<RustlsStream<S>>> + Send>>;

  fn connect(self, stream: S) -> Self::Future {
    match self.0 {
      None => {
        Box::pin(core::future::ready(Err(io::ErrorKind::InvalidInput.into())))
      }
      Some(c) => c
        .connector
